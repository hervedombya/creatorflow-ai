...
from google.genai import types  # en haut du fichier


# ==== ENV ====
# Charge .env AVANT de lire les variables
load_dotenv()
...
# Gemini client (nouveau SDK google-genai : pas de configure())
...
def generate_image_from_prompt(prompt: str, image_bytes: bytes) -> str:
    ...
    Utilise generate_content (multimodal) avec gemini-2.5-flash-image.
    """
    try:
        # On prépare la partie image pour Gemini
        image_part = types.Part.from_bytes(
            data=image_bytes,
            mime_type="image/jpeg",  # adapte si tu utilises du PNG
        )
        ...
        # On récupère la première image générée dans les parts
...
def generate_endpoint(payload: PromptInput):
    ...
    # ⚠️ en mode API, il faudra ajouter l’upload d’image (UploadFile)
    # pour l’instant, cette route ne passe pas d’image à Gemini.
    # On gère surtout le test local ci-dessous.

    raise HTTPException(status_code=400, detail="Use the local test or add image upload handling.")
